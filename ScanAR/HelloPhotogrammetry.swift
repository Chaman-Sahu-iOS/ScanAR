...
//            transform: Configuration.FeatureSensitivity.init)
    ...
    mutating func run() {
        ...
        let dataPath = docURL.appendingPathComponent("USDZ")
...
//        let documentsDirectoryy = pathss[0]
//        let docURLl = URL(string: documentsDirectoryy)!
        let imagePath = docURL.appendingPathComponent("Captures").appendingPathComponent("/")
        
        // Try to create the session, or else exit.
        var maybeSession: PhotogrammetrySession? = nil
        do {
            maybeSession = try PhotogrammetrySession(input: URL(fileURLWithPath: imagePath.path),
